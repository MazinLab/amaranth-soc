...
class SignatureTestCase(unittest.TestCase):
    def test_simple(self):
        sig = axi.Signature(addr_width=12, data_width=32)
        self.assertEqual(sig.addr_width, 12)
    ...
    #     sig = axi.Signature(addr_width=32, data_width=32,
    ...
    #         "stall": In(1),
    #         "lock":  Out(1),
    #         "cti":   Out(axi.CycleType),
    #         "bte":   Out(axi.BurstTypeExt),
    #     }).members)

    def test_create(self):
        sig   = axi.Signature(addr_width=39, data_width=16)
        iface = sig.create(path=("iface",))
        self.assertIsInstance(iface, axi.Interface)
        self.assertEqual(iface.addr_width, 39)
        self.assertEqual(iface.data_width, 16)
        self.assertEqual(iface.granularity, 8)
        self.assertEqual(iface.signature, sig)

    # def test_eq(self):
    #     self.assertEqual(axi.Signature(addr_width=32, data_width=8, features={"err"}),
    #                      axi.Signature(addr_width=32, data_width=8, features={"err"}))
    #     # different addr_width
    #     self.assertNotEqual(axi.Signature(addr_width=16, data_width=16, granularity=8),
    #                         axi.Signature(addr_width=32, data_width=16, granularity=8))
    #     # different data_width
    #     self.assertNotEqual(axi.Signature(addr_width=32, data_width=8, granularity=8),
    #                         axi.Signature(addr_width=32, data_width=16, granularity=8))
    #     # different granularity
    #     self.assertNotEqual(axi.Signature(addr_width=32, data_width=16, granularity=8),
    #                         axi.Signature(addr_width=32, data_width=16, granularity=16))
    #     self.assertNotEqual(axi.Signature(addr_width=32, data_width=16, granularity=8),
    #                         axi.Signature(addr_width=32, data_width=16))
    #     # different features
    #     self.assertNotEqual(axi.Signature(addr_width=32, data_width=16, granularity=8),
    #                         axi.Signature(addr_width=32, data_width=16, granularity=8,
    #                                            features={"err"}))
    ...
    def test_wrong_addr_width(self):
        with self.assertRaisesRegex(TypeError,
            ...
            axi.Signature(addr_width=-1, data_width=8)

    def test_wrong_data_width(self):
        with self.assertRaisesRegex(ValueError,
                r"Data width must be one of 8, 16, 32, 64, 128, 256, 512, 1024, 2048, not 7"):
            axi.Signature(addr_width=0, data_width=7)

    def test_wrong_features(self):
        with self.assertRaisesRegex(ValueError, r"'foo' is not a valid Feature"):
            axi.Signature(addr_width=0, data_width=8, features={"foo"})


class InterfaceTestCase(unittest.TestCase):
    def test_simple(self):
        iface = axi.Interface(addr_width=32, data_width=8, features={"last"})
        self.assertEqual(iface.addr_width, 32)
        self.assertEqual(iface.data_width, 8)
        self.assertEqual(iface.granularity, 8)
        self.assertEqual(iface.features, {axi.Feature.LAST})

    def test_set_map(self):
        iface = axi.Interface(addr_width=15, data_width=16)
    ...
    def test_get_map_none(self):
        iface = axi.Interface(addr_width=8, data_width=8)
        with self.assertRaisesRegex(AttributeError,
                r"axi.Interface\(.*\) does not have a memory map"):
            iface.memory_map
    ...
    def test_set_wrong_map_data_width(self):
        iface = axi.Interface(addr_width=30, data_width=32)
        with self.assertRaisesRegex(ValueError,
...
class DecoderTestCase(unittest.TestCase):
    def setUp(self):
        self.dut = axi.Decoder(addr_width=31, data_width=32)

    def test_add_align_to(self):
        sub_1 = axi.Interface(addr_width=14, data_width=32)
        sub_1.memory_map = MemoryMap(addr_width=16, data_width=8)
        sub_2 = axi.Interface(addr_width=14, data_width=32)
    ...
    def test_add_wrong(self):
        with self.assertRaisesRegex(TypeError,
                r"Subordinate bus must be an instance of axi\.Interface, not 'foo'"):
            self.dut.add(sub_bus="foo")

    def test_add_wrong_width_dense(self):
        sub = axi.Interface(addr_width=15, data_width=16)
        with self.assertRaisesRegex(ValueError,
    ...
    #     sub = axi.Interface(addr_width=15, data_width=64)
    #     with self.assertRaisesRegex(ValueError,
    ...
    #     sub = axi.Interface(addr_width=15, data_width=32, features={"err"})
    #     with self.assertRaisesRegex(ValueError,
    ...
    def test_add_wrong_out_of_bounds(self):
        sub = axi.Interface(addr_width=31, data_width=32)
        sub.memory_map = MemoryMap(addr_width=33, data_width=8)
        with self.assertRaisesRegex(ValueError,
            r"Address range 0x1000\.\.0x200001000 out of bounds for memory map spanning "
            r"range 0x0\.\.0x200000000 \(33 address bits\)"):
            self.dut.add(sub, addr=0x1000)


class DecoderSimulationTestCase(unittest.TestCase):
    def test_simple(self):
        dut = axi.Decoder(addr_width=30, data_width=32,
                               features={"lock", "burst", "last"})
        sub_1 = axi.Interface(addr_width=14, data_width=32)
        sub_1.memory_map = MemoryMap(addr_width=16, data_width=8)
        ...
        async def testbench(ctx):
            ...
            # ctx.set(dut.bus.cti, axi.CycleType.INCR_BURST)
            # ctx.set(dut.bus.bte, axi.BurstTypeExt.WRAP_4)
            # ctx.set(sub_1.ack, 1)
            ...
            # self.assertEqual(ctx.get(sub_2.cti), axi.CycleType.INCR_BURST.value)
            # self.assertEqual(ctx.get(sub_2.bte), axi.BurstTypeExt.WRAP_4.value)
    ...
    def test_addr_translate(self):
        class AddressLoopback(Elaboratable):
            def __init__(self, **kwargs):
                self.bus = axi.Interface(**kwargs)

            def elaborate(self, platform):
                m = Module()
                address_latch = Signal(self.bus.data_width)

                m.d.comb += self.bus.rdata.eq()
        ...
        dut = axi.Decoder(addr_width=20, data_width=32)
        loop_1 = AddressLoopback(addr_width=6, data_width=32)
        loop_1.bus.memory_map = MemoryMap(addr_width=8, data_width=8)
        self.assertEqual(dut.add(loop_1.bus, addr=0x10000),
                         (0x10000, 0x11000, 1))
        loop_2 = AddressLoopback(addr_width=5, data_width=32)
        loop_2.bus.memory_map = MemoryMap(addr_width=7, data_width=8, alignment=1)
        self.assertEqual(dut.add(loop_2.bus, addr=0x20000),
                         (0x20000, 0x21000, 1))
        loop_3 = AddressLoopback(addr_width=8, data_width=8)
        loop_3.bus.memory_map = MemoryMap(addr_width=8, data_width=8)
        self.assertEqual(dut.add(loop_3.bus, addr=0x30000, sparse=True),
                         (0x30000, 0x31000, 1))
        loop_4 = AddressLoopback(addr_width=8, data_width=8)
    ...
    #     dut = axi.Decoder(addr_width=3, data_width=32)
    #     sub = axi.Interface(addr_width=2, data_width=32)
...
#     def setUp(self):
#         self.dut = axi.Arbiter(addr_width=31, data_width=32, granularity=16,
#                                     features={"err"})

#     def test_add_wrong(self):
#         with self.assertRaisesRegex(TypeError,
#                 r"Initiator bus must be an instance of axi\.Interface, not 'foo'"):
#             self.dut.add(intr_bus="foo")

#     def test_add_wrong_addr_width(self):
#         intr = axi.Interface(addr_width=15, data_width=32, granularity=16,
...
#         intr = axi.Interface(addr_width=31, data_width=32, granularity=8,
#                                   features={"err"}, path=("intr",))
#         with self.assertRaisesRegex(ValueError,
...
#         intr = axi.Interface(addr_width=31, data_width=16, granularity=16,
#                                   features={"err"}, path=("intr",))
#         with self.assertRaisesRegex(ValueError,
...
#     def test_add_wrong_optional_output(self):
#         intr = axi.Interface(addr_width=31, data_width=32, granularity=16, path=("intr",))
#         with self.assertRaisesRegex(ValueError,
...
#     def test_simple(self):
#         dut = axi.Arbiter(addr_width=30, data_width=32, granularity=8,
#                                features={"err", "rty", "stall", "lock", "cti", "bte"})

#         intr_1 = axi.Interface(addr_width=30, data_width=32, granularity=8,
#                                     features={"err", "rty"}, path=("intr_1",))
#         dut.add(intr_1)
#         intr_2 = axi.Interface(addr_width=30, data_width=32, granularity=16,
...
#             self.assertEqual(ctx.get(dut.bus.cti), axi.CycleType.CLASSIC.value)
#             self.assertEqual(ctx.get(dut.bus.bte), axi.BurstTypeExt.LINEAR.value)
...
#             ctx.set(intr_2.cti, axi.CycleType.INCR_BURST)
#             ctx.set(intr_2.bte, axi.BurstTypeExt.WRAP_4)
#             await ctx.tick()
...
#             self.assertEqual(ctx.get(dut.bus.lock), 0)
#             self.assertEqual(ctx.get(dut.bus.cti), axi.CycleType.INCR_BURST.value)
#             self.assertEqual(ctx.get(dut.bus.bte), axi.BurstTypeExt.WRAP_4.value)
...
#     def test_lock(self):
#         dut = axi.Arbiter(addr_width=30, data_width=32, features={"lock"})
#         sig = axi.Signature(addr_width=30, data_width=32, features={"lock"})
...
#     def test_stall(self):
#         dut = axi.Arbiter(addr_width=30, data_width=32, features={"stall"})
#         sig = axi.Signature(addr_width=30, data_width=32, features={"stall"})
#         intr_1 = sig.create(path=("intr_1",))
#         dut.add(intr_1)
#         intr_2 = sig.create(path=("intr_2",))
#         dut.add(intr_2)
...
#         dut = axi.Arbiter(addr_width=30, data_width=32)
#         sig = axi.Signature(addr_width=30, data_width=32, features={"stall"})
#         intr_1 = sig.create(path=("intr_1",))
#         dut.add(intr_1)
#         intr_2 = sig.create(path=("intr_2",))
...
#         dut = axi.Arbiter(addr_width=30, data_width=32)
#         sig = axi.Signature(addr_width=30, data_width=32)
#         intr_1 = sig.create(path=("intr_1",))
#         dut.add(intr_1)
#         intr_2 = sig.create(path=("intr_2",))
#         dut.add(intr_2)
